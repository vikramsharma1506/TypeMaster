...
function displayResults() {
  ...
  if(timer=30){
    var speed = Math.floor((factor * wordsCorrect) / (30 / timer)); 
  }
  else{
    var speed = Math.floor((factor * wordsCorrect) / (60 / timer));
  }
  // Display speed and time in the modal
  document.getElementById("resultSpeed").innerText = speed;
  document.getElementById("resultAccuracy").innerText =  Math.floor((wordsCorrect/wordsSubmitted)*100);

  // Determine the message and title based on the speed
  var message, title;
  if(speed==0){
    message="AFK"
    title="Are you there???"
    ...
    message="Are you even Moving?"
    title="You're a Turtle!"
  }
  else if (speed < 30) {
    message = "You might want to practice a bit more.";
    title = "You're a Camel!";
  } else if (speed < 40) {
    ...
    message = "Stop Cheating,just kidding You're Insane";
    title = "You're a Hacker!";
  }

  // Display the message and title in the modal
